...
if (["prod", "production"].includes(config.env)) {
  ...
  config.url = env.PROD_URL;
  console.log(
    `Server has started as ${prod}. To start as ${dev} run command ${cmd(
      "env=dev npm start"
    )}.`
  );
} else {
  ...
  config.url = env.DEV_URL;
  console.log(
    `Server has started as ${dev}. To start as ${prod} run command ${cmd(
      "env=prod npm start"
    )}.`
  );
}
