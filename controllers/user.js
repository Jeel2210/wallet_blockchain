...
const updateUser = async (req, res, next) => {
	try {
		...
		if (!findUser) return errorMessage('Invalid user details. Please try again.', true);
		...
		if (!updateUser) errorMessage('Failed to update user details. Please try again.', true);
...
const getAllUsers = async (req, res, next) => {
	try {
		...
		let userDetails = await models.user.findAndCountAll({
			where,
			include: [{ model: models.wallet, as: 'wallet', attributes: { exclude: ['private_key'] }, }]
		});
		let wallet = new Wallet()
...
const sendMoney = async (req, res, next) => {
	try {
		...
		*/
		...
		//build a block
		let tDetails = {}
		let moneySend = await wallet.sendMoney(amount, payeerPublicKey, payeePublicKey);
		if (moneySend) {
			/**
			...
			*/
			...
			console.log("walletBalance=====================================================", walletPayeerBalance);
			// console.log("Payee", payeePublicKey);

			let walletPayeeBalance = await wallet.getbalance(req, payeePublicKey);

			console.log("walletBalance=====================================================", walletPayeerBalance, walletPayeeBalance);
			// Update user
...
const createSignature = async (req, res, next) => {
	try {
		...
		let data = new resPattern.successResponse({ signature, transactionHash }, 'ts');
		res.status(data.status).json(data);
	} catch (error) {
		console.log(`---> `, error);
		return next(new resPattern.failedResponse(errorMessage(error)));
...
const createBlock = async (req, res, next) => {
	try {
		...
		console.log("payeer name : " + payeerPublicKey + "Payee name : " + transactionHash, signature);
		...
		// let check = await wallet.checkBalance(req, payeerPublicKey, amount);
		// if (check === false) errorMessage('Insuficient balance', true);
		let chain = await wallet.addBlockData(transactionHash, payeerPublicKey, signature)
		console.log(chain);
...
const transactionHistory = async (req, res, next) => {
	try {
		...
		let chainInstance = chain.chain.filter(
			function (value) {
		...
		chainInstance = chainInstance.slice((page_number - 1) * page_size, page_number * page_size);


		let data = new resPattern.successResponse({ count, chainInstance }, 'Transaction History Successfully retrived');
		res.status(data.status).json(data);
	} catch (error) {
		console.log(`---> `, error);
		return next(new resPattern.failedResponse(errorMessage(error)));
...
const requestMoney = async (req, res, next) => {
	try {
		...
		**/
		...
		console.log("Requestor name : " + requestorPublicKey.slice(0, 8) + "Requested to name : " + requestedToPublicKey.slice(0, 8));
		...
		let requestedMoney = await models.request_money.create(createRequest, { transaction: req.tx });

		console.log("Requested Money details")

		if (!requestedMoney) errorMessage('Failed to send the Request.', true);

		let data = new resPattern.successResponse({ data: requestedMoney }, 'Request Sent successfully');
		res.status(data.status).json(data);
	} catch (error) {
		console.log(`---> `, error);
		return next(new resPattern.failedResponse(errorMessage(error)));
...
const ApproveToPay = async (req, res, next) => {
	try {
		...
		**/
		...
		let data = new resPattern.successResponse({ data: updateRequest }, 'Request updated successfully');
		res.status(data.status).json(data);
	} catch (error) {
		console.log(`---> `, error);
		return next(new resPattern.failedResponse(errorMessage(error)));
...
const getAllRequestMoneyHistory = async (req, res, next) => {
	try {
		...
		**/
		let publicKey = ''
		let requestorPublicKey = req.body.requestor_public_key || req.query.requestor_public_key;
		let requestedToPublicKey = req.body.requested_to_public_key || req.query.requested_to_public_key;
