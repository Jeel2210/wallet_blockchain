...
app.use(async (req, res, next) => {
	...
	res.end = async function (chunk) {
		...
		if (showBodyError && (response.success == undefined)) console.log(colorLog('UNABLE TO GET BODY ON res.end', 31, 47, 7))
		try {
...
app.use(function (err, req, res, next) {
	console.log(err);
	if (err?.name === 'ValidationError') err = new failedResponse(err.message, err.details);
...
server.listen(config.port || 3015, () => {
	...
	sequelize.sync().then((data) => {
		...
		console.log(`connected with database ${colorLog(data.config.database, 36, 0, 7)}.`)
	}).catch(err => {
